var params = getHashParams(),
  ...
  password,
  application_name =
    typeof params.appname == "undefined" ? "videochat" : params.appname,
  account_name = "qweasdqweasd",
  dialog,
  showLog = true,
  currentCall = null,
  outboundCall = null;
...
function onAuthResult(e) {
  ...
  if (e.result) {
    ...
    if (!$("div.alert.alert-danger").length)
      $("#authForm").prepend(
        '<div class="alert alert-danger" role="alert">Wrong username or password was specified</div>'
      );
    log("Code: " + e.code);
  }
}
...
function onCallConnected(e) {
  ...
  if ($("#cancelButton").length) {
    ...
    $(
      '<button type="button" class="btn btn-default" id="shareButton">Share Screen</button>'
    ).insertAfter("#cancelButton");
  } else {
    $("#callButton").replaceWith(
      '<button type="button" class="btn btn-danger" id="cancelButton">Disconnect</button>'
    );
    $(
      '<button type="button" class="btn btn-default" id="shareButton">Share Screen</button>'
    ).insertAfter("#cancelButton");
...
function onMicAccessResult(e) {
  ...
  if (e.result) {
    ...
    dialog.setMessage(
      "You have to allow access to your microphone to use the service"
    );
  }
}
...
function connect() {
  ...
  if (mode == "webrtc" && voxAPI.isRTCsupported()) {
    dialog = new BootstrapDialog({
      ...
      message:
        "Please click Allow to allow access to your camera and microphone",
      closable: false,
    });
    dialog.open();
  }
